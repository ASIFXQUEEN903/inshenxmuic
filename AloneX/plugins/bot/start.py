...
# ✅ Direct working image
config.START_IMG_URL = "https://files.catbox.moe/qi52co.jpg"
...
async def send_image_or_fallback(client, chat_id, photo, caption, reply_markup=None, disable_web_page_preview=True):
    """
    Try to send an image, if URL not valid or rejected by Telegram,
    fallback to sending caption as text (so bot never crashes).
    """
    try:
        ...
        try:
            ...
            print(f"[Fallback Send Error] {ex}")
            return None
...
async def start_pm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        if name.startswith("help"):
            ...
            return await send_image_or_fallback(
                client=client,
        ...
        if name.startswith("inf"):
            ...
            if await is_on_off(2):
                return await app.send_message(
                    chat_id=config.LOGGER_ID,
                    text=f"{message.from_user.mention} checked a <b>track information</b>.\n\n"
                         f"<b>ᴜsᴇʀ ɪᴅ :</b> <code>{message.from_user.id}</code>\n"
                         f"<b>ᴜsᴇʀɴᴀᴍᴇ :</b> @{message.from_user.username}"
                )
